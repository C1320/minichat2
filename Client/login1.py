...
from Client.MainPanel1 import MainWind_1
from cyq_sql import L_sql
from remenber_sql import reb_sql


class Login_1(QMainWindow):
    def __init__(self, parent=None):
        # 初始化继承父类
        super(Login_1, self).__init__(parent)
        try:
        ...
        if len(self.Account.text()) > 0 and len(self.passwd.text()) > 0:    # 判断账号密码不为空
            ...
            if self.bo:
                ...
                if self.c != self.Account.text():
                    ...
                    '''
                    self.main_gui = MainWind_1(self.name)  # 生成主窗口的实例
    ...
    def Edit(self):
        ...
        self.Avatar.setStyleSheet('''
                                              border-image: url(./imag/head2.jpg);
                                              ''')
        # 账号Label
