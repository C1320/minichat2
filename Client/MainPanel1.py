...
class MainWind_1(QMainWindow):
    def __init__(self, name, parent=None):
        super(MainWind_1, self).__init__(parent)
    ...
    def wallpaper(self, q):  # 壁纸
        if q.text() == '背景':
            ...
            if self.dig.exec_():
                ...
                # 设置对象名称（不用这个不显示？）
                self.chat_text.setObjectName('MainWindow')
                self.chat_text.setStyleSheet("#MainWindow{border-image:url(" + '{}'.format(self.bg) + ");}")
                print(self.bg)
    def client(self):  # 登录成功
        ...
        self.Online.setStyleSheet("background-color:LightBLue;")
        '''
        会话窗口
        '''
        self.str = ' '
        ...
        self.chat_label.setGeometry(300, 30, 700, 40)
        ...
        self.chat_text.setGeometry(300, 70, 700, 430)
        '''
        字体等控件
        '''
        ...
        # self.send_text.setReadOnly(False)  # 只读状态
